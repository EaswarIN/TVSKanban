class ProdPlanHeadersController < ApplicationController
  ...
  def create
    @prod_plan_header = ProdPlanHeader.new(prod_plan_header_params) 
    respond_to do |format|
      if @prod_plan_header.save
        format.html { redirect_to @prod_plan_header, notice: 'Prod plan header was successfully created.' }
        format.json { render :show, status: :created, location: @prod_plan_header }
      else
  ...
  private
    ...
    def prod_plan_header_params
      ...
      params.require(:prod_plan_header).permit(:plant, :work_center, :sfg_code, :sfg_desc, :sfg_uom, :trt_code, :day_req_qty_m2, :stock_qty, :sfg_plan_qty, :sfg_sequence, :bom_type, :schedule_no, :schedule_status,
 :safety_stock, :max_stock, :re_order_point, :action_status)
