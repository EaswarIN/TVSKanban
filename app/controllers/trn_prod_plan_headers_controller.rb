class TrnProdPlanHeadersController < ApplicationController
  ...
  def index
    ...
    @prod_plan_headers_list = TrnProdPlanHeader.select(:id, :trt_code, :sfg_code,:sfg_desc,:day_req_qty_m2, :re_order_point, :max_stock, :safety_stock, :stock_qty, :sfg_plan_qty, :sfg_sequence, :bom_type, :po_type, :schedule_no, :schedule_status,:alt_bom_no, :sap_plan_order).order(:sfg_sequence)
    ...
    .joins("LEFT JOIN trn_prod_plan_headers h ON trn_prod_plan_masters.trt_code = h.trt_code LEFT JOIN trn_planned_orders o ON o.sfg_code = trn_prod_plan_masters.sfg_code and trn_prod_plan_masters.plant = o.plant and o.plan_order_dt = current_date INNER JOIN mst_bom_hdrs bh ON o.sfg_code = bh.sfg_code and o.alt_bom_no = bh.alt_bom_no")
